...
// Macro for the VTKViewWindow class hierarchy implementation
FXIMPLEMENT(VTKViewWindow,FXMainWindow,VTKViewWindowMap,ARRAYNUMBER(VTKViewWindowMap))
...
// About
long VTKViewWindow::onCmdAbout(FXObject*,FXSelector,void*){
...
// Open
long VTKViewWindow::onCmdOpen(FXObject*,FXSelector,void*){
...
// Make new viewer
long VTKViewWindow::onCmdNewStructureViewer(FXObject*,FXSelector,void*){
    // Make new MDI child widget
    FXMDIChild* mdichild=new FXMDIChild(mdiclient,FXStringFormat("VTK Viewer #%d",no),winappicon,mdimenu,MDI_TRACKING,30,30,300,200);

    // Create new visual context
    FXGLVisual *glvisual=new FXGLVisual(getApp(),VISUAL_DOUBLEBUFFER);
    m_glvisual.push_back(glvisual);

    // create new VTK canvas
    FXVTKCanvas *canvas = new FXVTKCanvas(mdichild, glvisual, NULL, 0, LAYOUT_FILL_X | LAYOUT_FILL_Y);
...
// Make new animation
long VTKViewWindow::onCmdNewDeformedStructureViewer(FXObject*,FXSelector,void*){
  return 1;
}
...
long VTKViewWindow::onUpdMode(FXObject* sender,FXSelector,void*){
...
*/
long VTKViewWindow::onQueryMenu(FXObject* sender,FXSelector,void* ptr){
